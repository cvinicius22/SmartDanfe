...
def relatorio_excel(request):
    ...
    for nfe in nfes:
        ...
        try:
            ...
            modFrete = vol_qVol = vol_pesoB = vol_pesoL = ''
            ...
            notas_resumo.append({
                ...
                'Numero NF': nNF,
                'Data Emissao': dhEmi,
                'Data Saida/Entrada': dhSaiEnt,
                'Natureza Operacao': natOp,
                'Finalidade': finNFe,
                'Tipo NF (0=Entrada, 1=Saida)': tpNF,
                'Numero Controle': cNF,
                'Versao Processo': verProc,
                'Emitente (Nome)': emit_nome,
                'Emitente (CNPJ)': emit_cnpj,
                'Emitente (IE)': emit_ie,
                'Emitente (UF)': emit_uf,
                'Emitente (Municipio)': emit_mun,
                'Emitente (CEP)': emit_cep,
                'Destinatario (Nome)': dest_nome,
                'Destinatario (CNPJ)': dest_cnpj,
                'Destinatario (IE)': dest_ie,
                'Destinatario (E-mail)': dest_email,
                'Destinatario (UF)': dest_uf,
                'Destinatario (Municipio)': dest_mun,
                'Destinatario (CEP)': dest_cep,
                'Modalidade Frete': modFrete,
                'Qtde Volumes': vol_qVol,
                'Peso Bruto (kg)': vol_pesoB,
                'Peso Liquido (kg)': vol_pesoL,
            ...
            for det in root.findall('.//nfe:det', ns):
                ...
                vICMS_item = pICMS_item = vBC_item = '0'
                if ICMS is not None:
                    for child in ICMS:
                        if any(child.tag.endswith(t) for t in ('ICMS00', 'ICMS10', 'ICMS20', 'ICMS40', 'ICMS51')):
                ...
                itens.append({
                    ...
                    'Unidade': uCom,
                    'Quantidade': qCom,
                    'Valor Unitario (R$)': vUnCom,
                    'Valor Total Item (R$)': vProd,
                    'ICMS (R$)': vICMS_item,
                    'Aliquota ICMS (%)': pICMS_item,
    ...
    df_notas = pd.DataFrame(notas_resumo) if notas_resumo else pd.DataFrame({'Mensagem': ['Nenhuma NF-e com XML disponivel.']})
    df_itens = pd.DataFrame(itens)         if itens         else pd.DataFrame({'Mensagem': ['Nenhum item encontrado.']})
    df_xml   = pd.DataFrame(xml_completo)  if xml_completo  else pd.DataFrame({'Mensagem': ['Nenhuma NF-e com XML disponivel.']})
...
def checkout(request):
    ...
    def build_absolute_url(url_name):
        try:
            return f"{public_url}{reverse(url_name)}"
        except Exception as e:
    ...
    if not all([success_url, failure_url, pending_url, notification_url]):
        return render(request, 'nfe/error.html', {'message': 'URLs de retorno invalidas. Verifique as rotas.'})
    ...
    try:
        ...
        if preference_response.get('status') != 201:
            ...
            if cause:
                error += f" - {cause}"
            return render(request, 'nfe/error.html', {'message': f'Erro ao criar preferencia: {error}'})

        preference = preference_response.get('response', {})
        if 'id' not in preference:
            return render(request, 'nfe/error.html', {'message': 'Resposta invalida do Mercado Pago'})

        preference_id = preference['id']
        init_point    = preference.get('init_point')

    except Exception as e:
        logger.exception("Erro na criacao da preferencia")
    ...
    try:
        ...
        if preference_id:
            ...
            if payment_obj:
                ...
                # ✅ Se aprovado direto no brick, ja ativa a assinatura
                if status == 'approved':
                    _ativar_assinatura(payment_obj)
...
def payment_success(request):
    ...
    if preference_id:
        ...
        if payment:
            # Caso 1 — ja aprovado no banco (webhook chegou antes do redirect)
            if payment.status == 'APPROVED':
                _ativar_assinatura(payment)
                return redirect('dashboard')

            # Caso 2 — ainda pendente, consulta a API do MP para confirmar
            sdk = mercadopago.SDK(settings.MERCADOPAGO_ACCESS_TOKEN)
            try:
                mp_status = None
                mp_pay_id = payment_id

                # 2a. Tenta pelo payment_id vindo na URL
                if mp_pay_id:
                    info = sdk.payment().get(mp_pay_id)
                    if info['status'] == 200:
                        mp_status = info['response'].get('status')

                # 2b. Fallback: busca pela preference_id
                # (util quando o MP nao manda payment_id na URL — ex: Pix/boleto)
                if not mp_status:
                    search = sdk.payment().search({"filters": {"preference_id": preference_id}})
                    if search['status'] == 200:
                        results = search['response'].get('results', [])
                        if results:
                            mp_status = results[0].get('status')
                            mp_pay_id = results[0].get('id')

                if mp_status == 'approved':
                    payment.status     = 'APPROVED'
                    payment.payment_id = mp_pay_id
                    payment.save()
                    _ativar_assinatura(payment)
                    return redirect('dashboard')  # ✅ vai direto pro dashboard

            except Exception as e:
                logger.error(f"Erro ao consultar pagamento no MP: {e}")

    # Nao conseguiu confirmar — mostra tela de sucesso com botao pro dashboard
    return render(request, 'nfe/payment_success.html')


# ──────────────────────────────────────────────────────────────────────────── #
#  PAYMENT FAILURE / PENDING                                                    #
# ──────────────────────────────────────────────────────────────────────────── #
@login_required
...
def payment_webhook(request):
    ...
    if secret:
        ...
        if computed != hash_v1:
            logger.warning("Falha na validacao da assinatura do webhook")
            return JsonResponse({'status': 'ok'})

    try:
        data = json.loads(request.body)
        logger.info(f"Webhook dados: {data}")
    except Exception as e:
    ...
    logger.info(f"Payment ID recebido: {payment_id}")

    sdk = mercadopago.SDK(settings.MERCADOPAGO_ACCESS_TOKEN)
    try:
        payment_info = sdk.payment().get(payment_id)
        if payment_info['status'] != 200:
            logger.error(f"Erro na consulta ao MP: {payment_info}")
            return JsonResponse({'status': 'ok'})
        payment_data       = payment_info['response']
        status             = payment_data.get('status')
        preference_id      = payment_data.get('preference_id')
        external_reference = payment_data.get('external_reference')
        logger.info(f"Status: {status} | Preference: {preference_id} | ExtRef: {external_reference}")
    except Exception as e:
        logger.error(f"Erro ao consultar pagamento na API: {e}")
        return JsonResponse({'status': 'ok'})

    # Busca o pagamento — prioridades: preference_id > external_reference > payment_id
    payment = None
    if preference_id:
        payment = Payment.objects.filter(preference_id=preference_id).first()
    if not payment and external_reference:
        payment = Payment.objects.filter(
            external_reference=external_reference, status='PENDING'
        ).order_by('-created_at').first()
    if not payment:
        payment = Payment.objects.filter(payment_id=payment_id).first()

    if not payment:
        logger.warning("Pagamento nao encontrado no banco!")
        return JsonResponse({'status': 'ok'})

    logger.info(f"Status anterior: {payment.status}")
    payment.status     = status.upper()
    payment.payment_id = payment_id
    payment.save()
    logger.info(f"Status atualizado para: {payment.status}")
...
def payment_history(request):
    ...
    if profile.subscription_active and profile.subscription_until:
        status_str = 'Ativa' if profile.subscription_until > timezone.now() else 'Expirada'
        active_subscription = {
            'plan':            profile.plan,
    ...
    return render(request, 'nfe/payment_history.html', {
